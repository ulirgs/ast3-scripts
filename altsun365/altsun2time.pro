...
;    d1    - a limit value of the sun altitude for twilight (default=-9) 
;    d2    - another limit value of the sun altitude for twilight (default=-13) 
;  
;  OUTPUT:
...
pro altsun2time,file=file,alt1=alt2,alt2=alt2
;   DEFAULT VALUES AND ERROR CHECKING
if not keyword_set(file) then file = '2016_altsun365.dat' 
if not keyword_set(alt1) then alt1 = -9
if not keyword_set(alt2) then alt2 = -13

readcol,file,format='d,f,f,f,f',DJD, $
	sunAz, sunAlt ,moonAz ,moonAlt, mPhase

i_flat=where(sunAlt lt max([d1,d2]) and sunAlt gt min([d1,d2]))
